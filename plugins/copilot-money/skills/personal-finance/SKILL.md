...
description: >
  Domain knowledge for interpreting personal finance data from Copilot Money.
  ...
  net worth, savings, cash flow, or any Copilot Money output. Also use it when
...
<objective>
Provide vocabulary and interpretation frameworks so the copilot-money agent presents
...
- **Category accuracy:** Copilot Money auto-categorizes. Miscategorized
  transactions skew budget and cash flow data. If a large transaction seems
  miscategorized based on the merchant name, mention it.

## Copilot Money Amount Convention

In Copilot Money, positive amounts represent expenses (money going out) and
negative amounts represent income (money coming in). This is the opposite of
some other financial tools. Always clarify direction when presenting amounts:

- Label positive amounts as expenses/spending
- Label negative amounts as income/credits
- When showing net cash flow, explicitly state the direction

## Local Cache Limitations

The Copilot Money CLI reads from a local database cache. Important caveats:

- Only recent transactions are available (roughly the last few months,
  depending on what the app has synced).
- For older data, the user needs to open the Copilot Money app and scroll
  back in their transaction history to trigger a sync.
- Data freshness depends on when the app was last opened and synced.
- If the cache is missing or empty, the user needs to open the app first.

## Category Matching

Categories in Copilot Money are hierarchical (parent/child grouping). When
the user asks about a broad topic:

- "food" -> check "Restaurants", "Groceries", "Food & Drink", and any
  subcategories
- "transport" -> check "Transportation", "Gas", "Parking", "Auto", etc.
- "entertainment" -> check "Entertainment", "Subscriptions", etc.

Use `gfd-copilot-money categories --format json` to get the full category
tree and match broadly when the user's query is general.

## Pending Transactions

Some transactions are marked as pending. They may change amount or disappear
entirely. When presenting data:

- Flag pending transactions visually (e.g., "pending" label)
- Note that pending amounts may change
- Don't include pending transactions in firm totals without disclaiming
