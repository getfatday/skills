...
1. Prompt user for required fields: name, status (default active), created (default today).
...
3. **Enumerate collections:**
   - Glob `./Programs/{name}/initiatives/*/index.md`. Show count and links. Initiative path is a directory hub (`index.md`) because initiatives host their own epics collection.
...
1. Validate transition against allowed transitions:
   ...
   - active -> completed
   - paused -> completed
2. Terminal state: completed (no outbound transitions).
3. If invalid, reject with error message listing allowed transitions from current status.
4. If valid, update the status field and report the transition.
...
2. **On get:** glob child collections and display summary counts. For initiatives, glob `*/index.md` (directory hubs, not flat files) since initiatives host their own epics.
...
5. **Dual hosting awareness:** the initiative skill and epic skill must be aware that documents of their type can live under programs OR at root level. Both locations are valid.
